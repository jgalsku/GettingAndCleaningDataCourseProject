...
###########################################################################
# create second independent data set and write it as a table (txt)
###########################################################################

#create data set with the average of each variable for each activity and each subject.
mean_per_activity_subject <- tidy_data %>% group_by(activity, subject) %>% summarise(across(.cols = everything(), mean, .names = "average_{.col}"))
mean_per_activity_subject

#write txt file with second data set
setwd("C:/Users/yo/Dropbox/coursera/Getting_and_Cleaning_Data/final project/GettingAndCleaningDataCourseProject")
